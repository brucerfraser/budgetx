...
class one_transfer(one_transferTemplate):
  ...
  def fill_details(self,**event_args):
    d_a = [t for t in Global.TRANSACTIONS if t['transaction_id'] == self.item[0]][0]['date']
    ac_a = self.account_zero = [t for t in Global.TRANSACTIONS if t['transaction_id'] == self.item[0]][0]['account']
    ac_a = [a for a in Global.ACCOUNTS if a[1] == ac_a][0][0]
    am_a = self.amount_zero = [t for t in Global.TRANSACTIONS if t['transaction_id'] == self.item[0]][0]['amount']
    am_a = am_a/100
    ...
    if self.item[1]:
      self.date_picker_1.date = [t for t in Global.TRANSACTIONS if t['transaction_id'] == self.item[1]][0]['date']
      self.drop_down_1.selected_value = [t for t in Global.TRANSACTIONS if t['transaction_id'] == self.item[1]][0]['account']
      a = [t for t in Global.TRANSACTIONS if t['transaction_id'] == self.item[1]][0]['amount']
      a = a/100
