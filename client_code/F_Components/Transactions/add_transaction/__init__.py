from ._anvil_designer import add_transactionTemplate
from anvil import *
import anvil.server
import anvil.users
...
class add_transaction(add_transactionTemplate):
  ...
  def refresh_form(self,**event_args):
    ...
    self.item = {'date':date_new,'amount':0,
                 'description':'',
                 'category':None,
                 'account':None,'notes':'',
  ...
  def category_choose(self,**event_args):
    ...
    if self.item['category']:
      ...
      self.autocomplete_1.background = Global.CATEGORIES[self.item['category']]['colour']
